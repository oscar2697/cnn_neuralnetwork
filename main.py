    ...
    def inference(self, request: InferenceRequest):
        ...
        with torch.no_grad():
            ...
            for name, tensor in feature_maps.items():
                if tensor.dim() == 4:
                    aggredated_tensor = torch.mean(tensor, dim=1)
                    squeezed_tendor = aggredated_tensor.squeeze(0)
                    numpy_array = squeezed_tendor.cpu().numpy()
